...
public:
    ...
    double volatility(std::vector<double>& v){
        if(v.size()<10) return 0;
    ...
    void forward_bias(double aTech, double aCapex, double aOil){
        std::cout<<"\n=== FORWARD BIAS ===\n";
    ...
    void risk_engine(double alpha, double vol){
        std::cout<<"\n=== RISK ENGINE ===\n";
        ...
        if(vol > 2.0)
            std::cout<<"⚠️ HIGH VOL → Reduce position size\n";

        if(alpha > 0.5 && vol < 1.0)
            std::cout<<"✅ CLEAN TREND → Increase exposure\n";

        if(alpha > 0 && vol > 2.5)
            std::cout<<"⚠️ CHOPPY MARKET → Trade smaller\n";
    }
    ...
    void run(){
        ...
        // ===== FORWARD =====
        forward_bias(aTech, aCapex, aOil);

        // ===== RISK =====
        risk_engine(alpha, vol);
    }
};

int main(){
    Engine e;
    e.load("data/macro_data_v2.csv");
    e.run();
}
